...
if TYPE_CHECKING:
    from base.game_event import ButtonClickEvent, ClickEvent, RemovePlantCardFromBankEvent
from game.ui.plant_card import PlantCard
from utils.utils import create_ui_manager_with_theme, transform_coor_sys
...
class PlantSelectContainer:
    def __init__(self):
        ...
        # container的位置, 这将影响UI组件的位置
        self.container_pos = Vector2(0, 80)
        panel_rect = pygame.Rect(0, 0, self.container_size[0], self.container_size[1])
        panel_rect.topleft = tuple(self.container_pos)
        self.panel = pygame_gui.elements.UIPanel(
            relative_rect=panel_rect,
            starting_height=1,
            manager=self.ui_manager,
            object_id="#plant_select_container_panel"
        )
    ...
    def setup(self):
        ...
        start_fight_rect.bottomleft = (self.container_size[0] / 2 - start_fight_rect.width / 2, -25)
    ...
    def update(self, dt: float) -> None:
        ...
        if self.scroll_bar is not None and self.enable_scroll_bar:
            ...
            if self.scroll_bar.scrollable_height - self.scroll_bar.sliding_button.rect.height != 0:
                factor = (self.get_view_height() - self.container.height) / (
    ...
    def draw(self, screen: Surface, bgsurf=None, special_flags=0) -> None:
        ...
        screen.blit(self.container, self.container_pos)
        self.container.blit(self.background, Vector2(0, 0))
        for card in self.card_list:
            self.container.blit(card.image, card.world_pos + Vector2(0, -self.scroll_offset))
    ...
    def _on_click(self, event: 'ClickEvent'):
        # 限定点击范围
        if not pygame.Rect(self.container_pos.x, self.container_pos.y, self.container_size[0],
                           self.container_size[1]).collidepoint(event.mouse_pos):
            return
            # 不可见时不处理
        if not self.visible: return
        # 相对容器的鼠标坐标
        mouse_pos_in_container = transform_coor_sys(event.mouse_pos, self.container_pos)
        # 确定点击的是哪个PlantCard
        card = None
